...
def get_student_list():
    return [os.path.splitext(f)[0] for f in os.listdir(STUDENT_FOLDER) if f.endswith(".jpg")]
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("🛠️ Admin Panel"):
        ...
        remove_student_dropdown = gr.Dropdown(choices=get_student_list(), label="Remove Student")
        remove_btn = gr.Button("🗑️ Remove Student")
        remove_status = gr.Textbox(interactive=False)

        add_btn.click(add_student, inputs=[new_student_image, new_student_name], outputs=[add_status]).then(
            lambda: gr.Dropdown.update(choices=get_student_list()), None, remove_student_dropdown
        )
        remove_btn.click(remove_student, inputs=remove_student_dropdown, outputs=remove_status).then(
            lambda: gr.Dropdown.update(choices=get_student_list()), None, remove_student_dropdown
        )
